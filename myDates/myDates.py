...
def DateStrtoNum(datestr, dtype):
    ...
    if (dtype == 'min'):
        if (len(d) > 1):
            hr, minn = d[1].split(':')
            datenum = datetime.toordinal(datetime(int(year), int(month), int(day))) + 365 +  (int(hr)/24) + (int(minn)/(24*60))
            return datenum
        else:
            datenum = datetime.toordinal(datetime(int(year), int(month), int(day))) + 365
            return datenum
    elif (dtype == 'hr'):
        if (len(d) > 1):
            hr, minn = d[1].split(':')
